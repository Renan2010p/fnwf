...
auto main(int argc, char** argv) -> int {
        ...
        if (name == "warning") {
            ...
            sm.switch_state("extras", [&](fnwf::Engine& e) {
                return std::make_unique<fnwf::ExtrasState>(completed_nights,
                                                           save_data.infinite_power,
                                                           save_data.fast_nights,
                                                           achievements,
                                                           e);
            });
        }
    ...
    while (eng.keeps_running()) {
        ...
        fps_frames++;
        if (fps_timer >= 0.25) {
            fps_value = static_cast<int>((fps_frames / fps_timer) + 0.5);
            fps_timer = 0;
            fps_frames = 0;
        }
        ...
        if (cur && cur->is_done()) {
            if (state_name == "warning") {
                ...
                if (res == "start") {
                    ...
                    pending_night = std::min(completed_nights + 1, 6);
                    sm.switch_state("story", [n = pending_night](fnwf::Engine& e) {
                        return std::make_unique<fnwf::StoryState>(n, e);
                    });
                    state_name = "story";
                } else if (res == "night6") {
                    pending_night = 6;
            ...
            } else if (state_name == "extras" || state_name == "conquistas" ||
                       state_name == "options") {
                do_switch("menu");
                ...
                if (res == "win") {
                    if (pending_night >= 1) {
                        achievements.push_back("survive_n1");
                    }
                    if (pending_night >= 5) {
                        achievements.push_back("survive_n5");
                    }
                    if (pending_night >= 6) {
                        achievements.push_back("survive_n6");
                    }
                    completed_nights = std::max(completed_nights, pending_night);
                    has_seen_story = true;
                    fnwf::SaveManager::save_progress(completed_nights,
                                                     has_seen_story,
                                                     save_data.infinite_power,
                                                     save_data.fast_nights,
                                                     achievements);
