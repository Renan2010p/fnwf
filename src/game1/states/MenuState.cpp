...
#include <cmath>
namespace fnwf {
static float lerp(float a, float b, float t) { return a + (b - a) * std::min(1.0f, std::max(0.0f, t)); }
    ...
    : m_eng(eng), m_completed_nights(cn), m_has_seen_story(hss)
{
    ...
    DrawUtils::load_sprite(eng, "cedro"); DrawUtils::load_sprite(eng, "eser");
    DrawUtils::load_sprite(eng, "alice"); DrawUtils::load_sprite(eng, "Sonk"); DrawUtils::load_sprite(eng, "mafia");
    cedro_sprite = *DrawUtils::get_sprite("cedro"); eser_sprite = *DrawUtils::get_sprite("eser");
    alice_sprite = *DrawUtils::get_sprite("alice"); sonk_sprite = *DrawUtils::get_sprite("Sonk");
    mafia_logo = *DrawUtils::get_sprite("mafia");
    menu_animatronics = {eser_sprite, cedro_sprite, alice_sprite, sonk_sprite};
    current_anim_idx = std::rand() % 4;
    build_options();
...
void MenuState::build_options() {
    ...
    if (menu_page == "main") {
        ...
        if (m_has_seen_story || m_completed_nights > 0) {
            ...
            char buf[128]; std::snprintf(buf, sizeof(buf), "%s (%s %d)", Localization::get_text("continue").c_str(), Localization::get_text("night").c_str(), next);
            options.push_back({buf, "continue"});
        }
        if (m_completed_nights >= 5) options.push_back({Localization::get_text("night") + " 6", "night6"});
        if (m_completed_nights >= 5) options.push_back({Localization::get_text("more"), "more"});
...
void MenuState::handle_event(const Event& ev) {
    if (ev.type == EventType::KeyDown) {
        if (ev.key == 1073741906 || ev.key == 'w') { selected = ((selected - 2 + (int)options.size()) % (int)options.size()) + 1; SoundManager::play_sound("blip"); }
        else if (ev.key == 1073741905 || ev.key == 's') { selected = (selected % (int)options.size()) + 1; SoundManager::play_sound("blip"); }
        else if (ev.key == 13 || ev.key == 32) { auto r = handle_action(options[selected - 1].action); if (!r.empty()) { m_result = r; done = true; } }
    }
    if (ev.type == EventType::MouseButtonDown) {
        for (int i = 0; i < (int)options.size(); ++i) {
            ...
            if (ev.y >= y && ev.y <= y + 48 && ev.x >= 54 && ev.x <= 414) {
                ...
                if (!r.empty()) { m_result = r; done = true; }
            }
        }
    }
}
auto MenuState::handle_action(const std::string& action) -> std::string {
    ...
    if (action == "more") { menu_page = "more"; selected = 1; build_options(); return {}; }
    if (action == "back_menu") { menu_page = "main"; selected = 1; build_options(); return {}; }
...
void MenuState::draw(Engine& eng) {
    ...
    DrawUtils::rounded_texture(eng, cur, GameSettings::SCREEN_WIDTH - 640, 70 + bounce, 560, 560, 34, 4, 5, 9);
    ...
    for (int i = 0; i < (int)options.size(); ++i) {
        ...
        if (is_sel) eng.draw_rect(48, y - 17, 6, 48, 100, 180, 255);
        auto bri = (int)lerp(140, 255, is_sel ? 1.0f : 0.0f);
        DrawUtils::text(eng, options[i].label, 68, y - 5, 22, bri, bri, bri + (int)(20 * (is_sel ? 1 : 0)));
    }
    DrawUtils::text(eng, "[ UP / DOWN ] SELECT     [ ENTER ] PLAY     [ ESC ] BACK", 60, GameSettings::SCREEN_HEIGHT - 58, 13, 110, 130, 160, 200);
    DrawUtils::text(eng, "Five Nights With Friends", 60, GameSettings::SCREEN_HEIGHT - 38, 11, 80, 90, 110, 140);
}
} // namespace fnwf
