...
auto ConquistasState::draw(Engine& eng) -> void {
    ...
    DrawUtils::text(eng,
                    Localization::get_text("ach_title_screen"),
                    80,
                    60,
                    40,
                    TITLE_COLOR.r,
    ...
    for (int i = 0; i < (int)achievements_list.size(); ++i) {
        ...
        if (unlocked)
            eng.draw_rect(80, y - 10, SCREEN_WIDTH - 160, 80, 40, 40, 60, 255, false);

        DrawUtils::text(eng,
                        unlocked ? "\xe2\x98\x85" : "\xe2\x98\x86",
                        100,
                        y + 10,
                        40,
                        unlocked ? STAR_COLOR.r : 40,
                        unlocked ? STAR_COLOR.g : 40,
                        unlocked ? STAR_COLOR.b : 50);
    ...
    DrawUtils::text(eng,
                    Localization::get_text("ach_help"),
                    SCREEN_WIDTH / 2,
                    SCREEN_HEIGHT - 60,
                    16,
                    180,
