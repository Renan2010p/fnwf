...
bool GameOverState::is_done() const { return timer > 3.0; }

void GameOverState::update(double dt)
{
    timer += dt;
    if (timer > 0.5) { show_text = true; text_alpha = std::min(255, text_alpha + (int)(200 * dt)); }
}
...
{
    if (is_win) draw_win(eng);
    else draw_game_over(eng);
}
...
void GameOverState::draw_game_over(Engine& eng)
{
    ...
    if (text_alpha > 100)
    {
        DrawUtils::text(eng, Localization::get_text("news_title"), SCREEN_WIDTH / 2, py + 20, 18, 40, 40, 40, 255, true);
        eng.line(px + 20, py + 45, px + pw - 20, py + 45, 40, 40, 40, 255);
        DrawUtils::text(eng, Localization::get_text("news_headline_1"), SCREEN_WIDTH / 2, py + 70, 32, 30, 30, 30, 255, true);
        DrawUtils::text(eng, Localization::get_text("news_headline_2"), SCREEN_WIDTH / 2, py + 110, 38, 150, 20, 20, 255, true);
        ...
        for (int i = 0; i < (int)lines.size(); ++i)
            DrawUtils::text(eng, lines[i], SCREEN_WIDTH / 2, py + 165 + i * 22, 13, 50, 50, 50, 255, true);
    }

    DrawUtils::scanlines(eng, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, 15);
    if (timer > 3.0 && ((int)(timer * 2) % 2 == 1))
        DrawUtils::text(eng, Localization::get_text("press_any_key"), SCREEN_WIDTH / 2, SCREEN_HEIGHT - 50, 16, 100, 100, 100, 255, true);
}
...
void GameOverState::draw_win(Engine& eng)
{
    ...
    DrawUtils::text(eng, Localization::get_text("gameover_win"), SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 - 40, 80, 255, 255, 255, 255, true);
    ...
    {
        std::string nl = Localization::get_text("night") + " " + std::to_string(night);
        DrawUtils::text(eng, Localization::get_text("night_complete") + " " + nl, SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 + 50, 24, 200, 200, 200, 255, true);
        for (int i = 0; i < std::min(night, 7); ++i)
            DrawUtils::star(eng, SCREEN_WIDTH / 2 - 75 + i * 25, SCREEN_HEIGHT / 2 + 100, 10, STAR_COLOR.r, STAR_COLOR.g, STAR_COLOR.b);
    }

    if (timer > 3.5 && ((int)(timer * 2) % 2 == 1))
        DrawUtils::text(eng, Localization::get_text("press_any_key_continue"), SCREEN_WIDTH / 2, SCREEN_HEIGHT - 50, 16, 100, 100, 100, 255, true);
}
