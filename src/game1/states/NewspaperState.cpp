...
{
    timer += dt;
    if (fading_in) { fade_alpha = std::max(0, fade_alpha - (int)(100 * dt)); if (fade_alpha <= 0) fading_in = false; }
...
{
    if ((ev.type == EventType::KeyDown || ev.type == EventType::MouseButtonDown) && !fading_in && !fading_out && timer > 3.0)
...
void NewspaperState::draw(Engine& eng)
{
    ...
    DrawUtils::text(eng, Localization::get_text("newspaper_name"), px + 30, py + 20, 20, 40, 40, 40);
    eng.line(px + 20, py + 45, px + pw - 20, py + 45, 40, 40, 40, 255);
    DrawUtils::text(eng, Localization::get_text("newspaper_h1"), px + pw / 2, py + 80, 34, 20, 20, 20, 255, true);
    DrawUtils::text(eng, Localization::get_text("newspaper_h2"), px + pw / 2, py + 120, 16, 50, 50, 50, 255, true);

    auto txt = std::vector<std::string>{
        Localization::get_text("newspaper_p1"),
    ...
    if (timer > 3.0 && ((int)(timer * 2) % 2 == 1))
        DrawUtils::text(eng, Localization::get_text("click_continue"), SCREEN_WIDTH / 2, SCREEN_HEIGHT - 50, 16, 255, 255, 255, 255, true);
