...
void OptionsState::rebuild_options() {
    ...
    for (auto& r : resolutions)
        res_vals.push_back(std::to_string(r.first) + "x" + std::to_string(r.second));
    std::vector<std::string> quality_vals = {Localization::get_text("low"),
                                             Localization::get_text("high")};

    options.push_back({"resolution",
                       Localization::get_text("resolution"),
                       OptionItem::Toggle,
                       res_vals,
                       current_res_idx,
                       false,
    ...
    options.push_back({"fullscreen",
                       Localization::get_text("fullscreen"),
                       OptionItem::Bool,
                       {},
                       0,
                       is_fullscreen,
    ...
    options.push_back({"language",
                       Localization::get_text("language"),
                       OptionItem::Toggle,
                       {"Português", "English"},
                       (lang == "pt") ? 0 : 1,
                       false,
                       ...
                       100,
                       ""});
    options.push_back({"master_volume",
                       Localization::get_text("master_volume"),
                       OptionItem::Slider,
                       {},
                       0,
                       false,
                       ...
                       100,
                       ""});
    options.push_back({"sfx_volume",
                       Localization::get_text("sfx_volume"),
                       OptionItem::Slider,
                       {},
                       0,
                       false,
                       ...
                       100,
                       ""});
    options.push_back({"music_volume",
                       Localization::get_text("music_volume"),
                       OptionItem::Slider,
                       {},
                       0,
                       false,
                       ...
                       100,
                       ""});
    options.push_back({"show_fps",
                       Localization::get_text("show_fps"),
                       OptionItem::Bool,
                       {},
                       0,
                       show_fps,
                       ...
                       ""});
    options.push_back(
        {"vsync", Localization::get_text("vsync"), OptionItem::Bool, {}, 0, vsync, 0, 100, ""});
    options.push_back({"quality",
                       Localization::get_text("quality"),
                       OptionItem::Toggle,
                       quality_vals,
                       (SettingsManager::instance().quality == "low") ? 0 : 1,
                       false,
                       0,
                       100,
                       ""});
    options.push_back({"discord_rpc",
                       Localization::get_text("discord_rpc"),
                       OptionItem::Bool,
                       {},
                       0,
                       discord_rpc,
...
void OptionsState::update(double dt) {
    ...
    if (content_h > visible_h) {
        ...
        if (target_item_y < (int)scroll_offset)
    ...
    if (ev.type == EventType::KeyDown) {
        ...
        if (key == 1073741906 || key == 'w') {
            ...
            if (opt.type == OptionItem::Toggle) {
                opt.current =
                    ((opt.current + dir) % (int)opt.values.size() + (int)opt.values.size()) %
                    (int)opt.values.size();
                if (opt.id == "language") {
                ...
                opt.slider_value =
                    std::max(0, std::min(opt.slider_max, opt.slider_value + dir * 5));
                SoundManager::play_sound("blip");
            }
    ...
    for (int i = 0; i < max_options; ++i) {
        ...
        if (my >= y - 9 && my <= y + 40 && mx >= panel_x + 25 && mx <= panel_x + panel_w - 25) {
            ...
            if (opt.type == OptionItem::Toggle) {
                if (mx < value_x) {
                    opt.current =
                        ((opt.current - 1) % (int)opt.values.size() + (int)opt.values.size()) %
                        (int)opt.values.size();
                } else {
                    opt.current = (opt.current + 1) % (int)opt.values.size();
                }
...
void OptionsState::apply_settings() {
    ...
    for (auto& opt : options) {
        ...
        } else if (opt.id == "language")
            s.language = (opt.current == 0) ? "pt" : "en";
        else if (opt.id == "show_fps")
            s.show_fps = opt.value;
        else if (opt.id == "vsync") {
            s.vsync = opt.value;
            m_eng.set_vsync(opt.value);
        } else if (opt.id == "quality") {
            s.quality = (opt.current == 0) ? "low" : "high";
            DrawUtils::set_render_quality(s.quality);
        } else if (opt.id == "discord_rpc")
            s.discord_rpc = opt.value;
        else if (opt.id == "master_volume") {
            s.master_volume = opt.slider_value;
            SoundManager::set_master_volume(opt.slider_value);
        } else if (opt.id == "sfx_volume") {
            s.sfx_volume = opt.slider_value;
            SoundManager::set_sfx_volume(opt.slider_value);
        } else if (opt.id == "music_volume") {
            s.music_volume = opt.slider_value;
            SoundManager::set_music_volume(opt.slider_value);
        }
    }
...
void OptionsState::draw(Engine& eng) {
    ...
    for (int i = 0; i < max_options; ++i) {
        ...
        if (opt.type == OptionItem::Toggle) {
            ...
            std::string val_text =
                opt.value ? Localization::get_text("on") : Localization::get_text("off");
            DrawUtils::text(eng, val_text, 500, y + 2, 22, val_r, val_g, val_b);
        } else if (opt.type == OptionItem::Slider) {
            int bar_x = 500;
            int bar_w = 200;
            ...
            DrawUtils::text(
                eng, vol_buf, bar_x + bar_w + 15, y + 2, 20, tc, tc, tc + (int)(20 * glow));
        }
    }

    DrawUtils::text(
        eng, Localization::get_text("help_input"), 70, SCREEN_HEIGHT - 55, 14, 90, 110, 140, 180);
