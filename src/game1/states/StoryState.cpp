...
StoryState::StoryState(int night_, Engine& eng) : m_eng(eng), night(night_)
{
    ...
    DrawUtils::load_sprite(eng, "mafia"); unknown_avatar = *DrawUtils::get_sprite("mafia");
    build_messages();
}
...
void StoryState::build_messages()
{
    ...
    auto row = [&](const std::string& user, const std::string& avatar, std::array<int,3> color, const std::string& key) -> Message {
    ...
    else if (night >= 2 && night <= 5)
    {
        ...
        messages.push_back(row("Cedro", "cedro", {140,110,80}, "m2"));
        messages.push_back({"title", "", {}, {}, Localization::get_text("night") + " " + std::to_string(night)});
        int i = 3;
        while (has_msg("m" + std::to_string(i)))
        {
            std::string speaker = ((i % 2) == 0) ? "Cientista" : "Cedro";
            std::string avatar = (speaker == "Cedro") ? "cedro" : "renan";
            auto color = (speaker == "Cedro") ? std::array<int,3>{140,110,80} : std::array<int,3>{100,180,255};
...
auto StoryState::wrap_text(const std::string& text, int max_width, int font_size) -> std::vector<std::string>
{
    ...
    for (auto& w : words)
    {
        ...
        else { if (!current.empty()) { lines.push_back(current); current = w; } else { lines.push_back(w); current.clear(); } }
    }
...
void StoryState::update_scroll_target()
{
    ...
    {
        auto& msg = messages[i];
        if (msg.type == "title") { th += 50; prev.clear(); }
        else
        {
            auto lines = wrap_text(msg.text, cwl, 14);
            if (prev == msg.user) th += 24 * (int)lines.size();
            else th += (prev.empty() ? 0 : 8) + 22 + 24 * (int)lines.size();
            prev = msg.user;
        }
    }
...
{
    timer += dt;
    if (phase == 0) { fade_alpha = std::max(0, fade_alpha - (int)(250 * dt)); if (fade_alpha <= 0) phase = 1; }
    else if (phase == 1 && visible_messages == 0) { visible_messages = 1; update_scroll_target(); }
    else if (phase == 2) { fade_alpha = std::min(255, fade_alpha + (int)(200 * dt)); if (fade_alpha >= 255) done = true; }
...
void StoryState::handle_event(const Event& ev)
{
    ...
    {
        if (ev.type == EventType::KeyDown && ev.key != 13 && ev.key != 32) return;
        if (phase == 0) { phase = 1; fade_alpha = 0; }
        else if (phase == 1)
        {
            if (visible_messages < (int)messages.size()) { visible_messages++; update_scroll_target(); SoundManager::play_sound("notification"); }
...
void StoryState::draw(Engine& eng)
{
    ...
    if (phase >= 1) draw_discord_ui(eng);
    ...
    if (phase == 1)
    {
        ...
        if (blink)
        {
            std::string txt = (visible_messages < (int)messages.size()) ? Localization::get_text("story_skip") : Localization::get_text("story_start_night");
            DrawUtils::text(eng, txt, SCREEN_WIDTH / 2, SCREEN_HEIGHT - 20, 12, 130, 130, 140, 255, true);
        }
    }
...
void StoryState::draw_discord_ui(Engine& eng)
{
    ...
    auto chans = {Localization::get_text("chan_general"), Localization::get_text("chan_announcements"),
                  Localization::get_text("chan_security"), Localization::get_text("chan_cameras")};
    int idx = 0;
    for (auto& ch : chans)
    {
        ++idx;
        ...
        if (idx == 3) eng.draw_rect(8, y - 4, sw - 16, 28, 60, 63, 69, 255);
    ...
    for (int i = 0; i < visible_messages && i < (int)messages.size(); ++i)
    {
        ...
        else
        {
            ...
            {
                for (auto& line : lines) { DrawUtils::text(eng, line, cx + 75, my, 14, 220, 220, 220); my += 24; }
            }
            else
            {
                my += prev_user.empty() ? 0 : 8;
                int av_size = 40;
                eng.circle(cx + 20 + av_size/2, my + av_size/2, av_size/2, 80, 80, 90, 255, true);
                auto av = (msg.avatar == "cedro") ? cedro_avatar : renan_avatar;
                eng.draw_texture(av, cx + 20, my, av_size, av_size);
                DrawUtils::text(eng, msg.user, cx + 75, my, 14, msg.color[0], msg.color[1], msg.color[2]);
                my += 22;
                for (auto& line : lines) { DrawUtils::text(eng, line, cx + 75, my, 14, 220, 220, 220); my += 24; }
            }
            prev_user = msg.user;
    ...
    DrawUtils::text(eng, Localization::get_text("discord_input"), cx + 30, iy + 13, 13, 100, 100, 110);
}
