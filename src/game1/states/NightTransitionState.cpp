...
NightTransitionState::NightTransitionState(int night_, Engine& eng) : m_eng(eng), night(night_) {
    ...
    if (night == 2) {
        raw = {
            {Localization::get_text("discord_server"), Localization::get_text("transition_n2_m0")},
        ...
        raw = {
            {Localization::get_text("discord_server"), Localization::get_text("transition_n3_m0")},
    ...
    for (auto& r : raw) {
        ...
        m.color = (r.user != "Renan") ? std::array<int, 3>{140, 110, 80}
...
auto NightTransitionState::update(double dt) -> void {
    ...
    if (phase == 0) {
        ...
        fade_alpha = std::min(255, fade_alpha + (int)(250 * dt));
        if (fade_alpha >= 255)
            done = true;
    }
...
auto NightTransitionState::draw(Engine& eng) -> void {
    ...
    if (phase == 1) {
        ...
        if (blink) {
            std::string txt =
                (visible_messages < (int)messages.size())
                    ? Localization::get_text("story_skip")
                    : Localization::get_text("story_start_night") + " " + std::to_string(night);
            DrawUtils::text(
                eng, txt, SCREEN_WIDTH / 2, SCREEN_HEIGHT - 20, 11, 130, 130, 140, 255, true);
        }
    }
    if (fade_alpha > 0)
...
auto NightTransitionState::draw_discord(Engine& eng) -> void {
    ...
    DrawUtils::text(eng,
                    Localization::get_text("discord_input"),
                    cx + 30,
                    SCREEN_HEIGHT - 52,
                    13,
                    100,
