...
static int digit_from_event(const Event& ev) {
    ...
    if (k >= '0' && k <= '9')
        return k - '0';
    auto kp_map = std::unordered_map<int, int>{{1073741922, 0},
                                               {1073741913, 1},
                                               {1073741914, 2},
                                               {1073741915, 3},
                                               {1073741916, 4},
...
auto CustomNightState::get_ai_cap() const -> int {
...
void CustomNightState::handle_event(const Event& ev) {
    if (ev.type == EventType::KeyDown) {
        ...
        if (digit >= 0) {
            ...
            if ((int)secret_code_buffer.size() > 4)
                secret_code_buffer = secret_code_buffer.substr(secret_code_buffer.size() - 4);
        ...
        if (selected <= 4) {
            ...
            if (change != 0) {
                int cap = get_ai_cap();
                ai_levels[selected - 1] =
                    std::max(0, std::min(cap, ai_levels[selected - 1] + change));
                current_preset = 0;
                SoundManager::play_sound("blip");
            }
        }
    ...
    if (ev.type == EventType::MouseButtonDown) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (mx >= r[0] && mx <= r[0] + r[2] && my >= r[1] && my <= r[1] + r[3]) {
                int cap = get_ai_cap();
                ai_levels[i] = std::max(0, std::min(cap, ai_levels[i] + 1));
                current_preset = 0;
                selected = i + 1;
                SoundManager::play_sound("blip");
            }
        ...
        if (mx >= preset_prev[0] && mx <= preset_prev[0] + preset_prev[2] && my >= preset_prev[1] &&
            my <= preset_prev[1] + preset_prev[3])
            cycle_preset(-1);
        if (mx >= preset_next[0] && mx <= preset_next[0] + preset_next[2] && my >= preset_next[1] &&
            my <= preset_next[1] + preset_next[3])
            cycle_preset(1);
        if (mx >= ready_rect[0] && mx <= ready_rect[0] + ready_rect[2] && my >= ready_rect[1] &&
            my <= ready_rect[1] + ready_rect[3]) {
            if (secret_code_pending) {
                secret_code_pending = false;
                unlock_secret_mode();
                SoundManager::play_sound("select");
...
void CustomNightState::draw(Engine& eng) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        int btn_r = (glow > 0.2) ? 150 : 90, btn_g = (glow > 0.2) ? 170 : 90,
            btn_b = (glow > 0.2) ? 210 : 105;
        ...
        DrawUtils::text(
            eng, std::to_string(lvl), cx, r[1] + 360, 52, val_r, val_g, val_b, 255, true);
    ...
    eng.draw_rect(
        preset_prev[0], preset_prev[1], preset_prev[2], preset_prev[3], 120, 120, 140, 255, false);
    DrawUtils::text(eng,
                    "<<",
                    preset_prev[0] + preset_prev[2] / 2,
                    preset_prev[1] + preset_prev[3] / 2,
                    28,
                    arrow_r,
                    arrow_g,
                    arrow_b,
    ...
    DrawUtils::text(eng,
                    p_name,
                    preset_label[0] + preset_label[2] / 2,
                    preset_label[1] + preset_label[3] / 2,
                    label_size,
                    255,
                    255,
                    120,
    ...
    eng.draw_rect(
        preset_next[0], preset_next[1], preset_next[2], preset_next[3], 120, 120, 140, 255, false);
    DrawUtils::text(eng,
                    ">>",
                    preset_next[0] + preset_next[2] / 2,
                    preset_next[1] + preset_next[3] / 2,
                    28,
                    arrow_r,
                    arrow_g,
                    arrow_b,
    ...
    if (rs) {
        ...
        eng.draw_rect(
            ready_rect[0], ready_rect[1], ready_rect[2], ready_rect[3], 120, 120, 130, 255, false);

    std::string ready_label = secret_code_pending ? Localization::get_text("apply_code")
                                                  : Localization::get_text("ready");
    std::string prefix = rs ? ">> " : "";
    DrawUtils::text(eng,
                    prefix + ready_label,
                    ready_rect[0] + ready_rect[2] / 2,
                    ready_rect[1] + ready_rect[3] / 2,
                    40,
                    rs ? 255 : 150,
                    rs ? 255 : 150,
                    rs ? 255 : 160,
    ...
    DrawUtils::text(eng,
                    Localization::get_text("custom_controls_help"),
                    SCREEN_WIDTH / 2,
                    SCREEN_HEIGHT - 22,
                    14,
                    100,
