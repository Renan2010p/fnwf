...
LoadingState::LoadingState(Engine& eng)
    : m_eng(eng)
{
    int tip_idx = (std::rand() % 12) + 1;
...
void LoadingState::update(double dt)
{
    ...
    if (timer < 0.5) font_alpha = (int)((timer / 0.5) * 255);
    else if (duration - timer < 0.5) font_alpha = (int)(((duration - timer) / 0.5) * 255);
    else font_alpha = 255;

    if (timer >= duration)
    {
        done = true;
        if (next_factory) next_state = next_factory(m_eng);
    }
}
...
void LoadingState::draw(Engine& eng)
{
    ...
    int ox = (std::rand() % 100 < 5) ? (std::rand() % 9 - 4) : 0;
    DrawUtils::text(eng, tip, SCREEN_WIDTH / 2 + ox, SCREEN_HEIGHT / 2, 18, 160, 160, 170, font_alpha, true);

    int lx = SCREEN_WIDTH - 80, ly = SCREEN_HEIGHT - 80, radius = 20;
    for (int i = 0; i < 8; ++i)
    {
        double angle = std::fmod(loader_angle, 360.0) * 3.14159 / 180.0 + i * 45.0 * 3.14159 / 180.0;
    ...
    DrawUtils::text(eng, Localization::get_text("loading"), lx, ly + 35, 14, 100, 100, 110, 255, true);
}
