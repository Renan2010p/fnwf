...
{
    timer += dt;
    if (!show_six && timer > 2.0) { hour = 6; show_six = true; }
    if (show_six && !played_chime) { SoundManager::play_sound("noite_concluida"); played_chime = true; }
    if (timer > 10.0) done = true;
}

void SixAMState::handle_event(const Event& ev)
{
    if (timer > 2.0 && (ev.type == EventType::KeyDown || ev.type == EventType::MouseButtonDown)) done = true;
}
...
void SixAMState::draw(Engine& eng)
{
    ...
    if (timer > 8.0 && ((int)(timer * 2) % 2 == 1))
        DrawUtils::text(eng, Localization::get_text("press_any_key"), cx, SCREEN_HEIGHT - 50, 16, 150, 150, 150, 255, true);
}
