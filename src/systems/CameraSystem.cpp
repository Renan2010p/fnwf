...
auto CameraSystem::get_bottom_toggle_rects() const -> std::pair<std::array<int,4>, std::array<int,4>>
{
    using namespace GameSettings;
...
{
    if (is_mask_open || is_mask_animating) return;
    is_open = !is_open;
    is_animating = true;
    if (is_open) static_timer = static_duration;
}

void CameraSystem::toggle_mask()
{
    if (is_open || is_animating || is_mask_animating) return;
...
{
    if (!is_mask_open && mask_anim_progress <= 0.0) return;
    if (is_mask_animating && !is_mask_open) return;
    is_mask_open = false;
    is_mask_animating = true;
}
...
auto CameraSystem::is_fully_open() const -> bool { return is_open && anim_progress >= 0.99; }
auto CameraSystem::is_visible() const -> bool { return anim_progress > 0.01; }
...
auto CameraSystem::check_toggle_click(int mx, int my) -> bool
{
    ...
    {
        if (!is_animating && !is_mask_open && !is_mask_animating) toggle();
        return true;
    }
    return false;
}
...
auto CameraSystem::check_mask_toggle_click(int mx, int my) -> bool
{
    ...
    if (mx >= rx && mx <= rx + rw && my >= ry && my <= ry + rh)
    {
        if (!is_mask_animating && !is_open && !is_animating) toggle_mask();
        return true;
    }
    return false;
}
...
auto CameraSystem::check_mouse_trigger(int mouse_x, int mouse_y) -> bool
{
    ...
    bool in_mask = mouse_x >= mx_m && mouse_x <= mx_m + mw_m && mouse_y >= my_m - 14 && mouse_y <= my_m - 14 + gesture_h;
    bool in_cam = mouse_x >= mx_c && mouse_x <= mx_c + mw_c && mouse_y >= my_c - 14 && mouse_y <= my_c - 14 + gesture_h;
    ...
    else if (!in_cam) mouse_in_cam_zone = false;
    ...
    else if (!in_mask) mouse_in_mask_zone = false;
...
{
    if (static_timer > 0) static_timer -= dt;

    double target = is_open ? 1.0 : 0.0;
    if (anim_progress != target)
    {
        is_animating = true;
        if (anim_progress < target) anim_progress = std::min(target, anim_progress + anim_speed * dt);
        else anim_progress = std::max(target, anim_progress - anim_speed * dt);
    }
    ...
    {
        is_mask_animating = true;
        if (mask_anim_progress < mask_target) mask_anim_progress = std::min(mask_target, mask_anim_progress + mask_anim_speed * dt);
        else mask_anim_progress = std::max(mask_target, mask_anim_progress - mask_anim_speed * dt);
    }
    else is_mask_animating = false;
}

void CameraSystem::draw(Engine& eng, const std::unordered_map<std::string, std::string>& anim_positions, int foxy_stage)
{
    auto [mask_rect, monitor_rect] = get_bottom_toggle_rects();
    ...
    if (!is_mask_open && !is_visible())
    {
        ...
        eng.draw_rect(mask_rect[0], mask_rect[1], mask_rect[2], mask_rect[3], 220, glow, 60, 255, false);
        DrawUtils::text(eng, "MASK", mask_rect[0] + mask_rect[2] / 2, mask_rect[1] + mask_rect[3] / 2, 12, 230, 160, 80, 255, true);
    }
    ...
    if (!is_visible() && !is_mask_open)
    {
        ...
        eng.draw_rect(monitor_rect[0], monitor_rect[1], monitor_rect[2], monitor_rect[3], 12, 18, 12, 220);
        eng.draw_rect(monitor_rect[0], monitor_rect[1], monitor_rect[2], monitor_rect[3], 40, edge_g, 90, 255, false);
        DrawUtils::text(eng, "MONITOR", monitor_rect[0] + monitor_rect[2] / 2, monitor_rect[1] + monitor_rect[3] / 2, 12, 120, edge_g, 160, 255, true);
    }

    if (mask_anim_progress > 0.01) draw_mask_overlay(eng);
    if (is_visible())
    {
        if (is_fully_open()) draw_monitor(eng, anim_positions, foxy_stage);
        else draw_monitor_animation(eng);
    }
}
...
void CameraSystem::draw_mask_overlay(Engine& eng)
{
    ...
    eng.draw_rect(0, y_top, SCREEN_WIDTH, std::max(0, eye_y - y_top), mask_r, mask_g, mask_b, mask_a);
    eng.draw_rect(0, eye_y + eye_h, SCREEN_WIDTH, std::max(0, y_bottom - (eye_y + eye_h)), mask_r, mask_g, mask_b, mask_a);
    ...
    eng.draw_rect(right_eye_x + eye_w, eye_y, std::max(0, SCREEN_WIDTH - (right_eye_x + eye_w)), eye_h, mask_r, mask_g, mask_b, mask_a);
...
void CameraSystem::draw_monitor(Engine& eng, const std::unordered_map<std::string, std::string>& anim_positions, int foxy_stage)
{
    using namespace GameSettings;
    ...
    else
    {
        ...
        DrawUtils::text(eng, "CAM " + current_cam, 30, 30, 20, CAM_OUTLINE.r, CAM_OUTLINE.g, CAM_OUTLINE.b);
...
void CameraSystem::draw_camera_view(Engine& eng, const std::unordered_map<std::string, std::string>& anim_positions, int foxy_stage)
{
    using namespace GameSettings;
    int cx = SCREEN_WIDTH / 2, cy = SCREEN_HEIGHT / 2 - 30;
    if (current_cam == "1A") draw_show_stage(eng, cx, cy, anim_positions);
    else if (current_cam == "1B") draw_dining_area(eng, cx, cy, anim_positions);
    else if (current_cam == "1C") draw_backstage(eng, cx, cy, anim_positions);
    else if (current_cam == "5") draw_sonk_cove(eng, cx, cy, foxy_stage);
    else if (current_cam == "2A" || current_cam == "4A") draw_hallway(eng, cx, cy, anim_positions, current_cam);
    else if (current_cam == "2B" || current_cam == "4B") draw_hall_corner(eng, cx, cy, anim_positions, current_cam);
    else if (current_cam == "3") draw_supply_closet(eng, cx, cy, anim_positions);
}

void CameraSystem::draw_show_stage(Engine& eng, int cx, int cy, const std::unordered_map<std::string, std::string>& ap)
{
    eng.draw_rect(cx - 300, cy - 150, 600, 350, 40, 30, 35);
    auto it = ap.find("cedro");
    if (it != ap.end() && it->second == "1A") DrawUtils::animatronic_face(eng, "cedro", cx - 130, cy - 40, 100, 120);
    it = ap.find("eser");
    if (it != ap.end() && it->second == "1A") DrawUtils::animatronic_face(eng, "eser", cx + 30, cy - 40, 100, 120);
}

void CameraSystem::draw_dining_area(Engine& eng, int cx, int cy, const std::unordered_map<std::string, std::string>& ap)
{
...
void CameraSystem::draw_backstage(Engine& eng, int cx, int cy, const std::unordered_map<std::string, std::string>& ap)
{
    eng.draw_rect(cx - 300, cy - 150, 600, 350, 25, 20, 28);
    auto it = ap.find("cedro");
    if (it != ap.end() && it->second == "1C") DrawUtils::animatronic_face(eng, "cedro", cx - 60, cy - 40, 120, 150);
}

void CameraSystem::draw_hallway(Engine& eng, int cx, int cy, const std::unordered_map<std::string, std::string>& ap, const std::string& cam_id)
{
...
void CameraSystem::draw_hall_corner(Engine& eng, int cx, int cy, const std::unordered_map<std::string, std::string>& ap, const std::string& cam_id)
{
...
void CameraSystem::draw_supply_closet(Engine& eng, int cx, int cy, const std::unordered_map<std::string, std::string>& ap)
{
    eng.draw_rect(cx - 300, cy - 150, 600, 350, 28, 25, 30);
    auto it = ap.find("cedro");
    if (it != ap.end() && it->second == "3") DrawUtils::animatronic_face(eng, "cedro", cx - 60, cy - 40, 120, 150);
}
...
void CameraSystem::draw_sonk_cove(Engine& eng, int cx, int cy, int foxy_stage)
{
    ...
    if (foxy_stage == 1) label = "RUSTLING...";
    else if (foxy_stage == 2) label = "MOVING...";
    else if (foxy_stage >= 3) label = "GONE";
...
void CameraSystem::draw_map(Engine& eng, const std::unordered_map<std::string, std::string>&)
{
    ...
    auto cam_positions = std::unordered_map<std::string, std::pair<int,int>>{
        {"1A", {map_w/2, 40}}, {"1B", {map_w/2, 80}}, {"1C", {60, 100}},
        {"5", {145, 105}}, {"2A", {90, 150}}, {"2B", {90, 210}},
        {"3", {50, 160}}, {"4A", {map_w-90, 150}}, {"4B", {map_w-90, 210}}
    };

    auto connections = std::vector<std::pair<std::string,std::string>>{
        {"1A","1B"}, {"1B","1C"}, {"1C","5"}, {"1B","2A"}, {"2A","2B"},
        {"2A","3"}, {"1B","4A"}, {"4A","4B"}
    };
    ...
    for (auto& [cam_id, pos] : cam_positions)
    {
        ...
        eng.draw_rect(pos.first - btn_w/2, pos.second - btn_h/2, btn_w, btn_h, bg_r, bg_g, bg_b);
        eng.draw_rect(pos.first - btn_w/2, pos.second - btn_h/2, btn_w, btn_h, 0, 255, 0, 255, false);
