...
{
    return {
        static_cast<std::uint8_t>(std::max(0, std::min(255, static_cast<int>(c.r * factor)))),
...
auto Office::project_depth(double nx, double z, double height) -> std::tuple<int, int, double>
{
    ...
    double floor_y = bw_bottom + static_cast<int>((GameSettings::SCREEN_HEIGHT - bw_bottom) * std::pow(zc, 1.2));
...
void Office::precalculate_projection()
{
    ...
    for (int sx = 0; sx < screen_w; sx += slice_w)
    {
        ...
        double src_w_d = slice_w * (pixels_per_radian * std::pow(std::cos(theta), 2) / focal_length);
        projection_data.push_back({
            sx, theta,
...
void Office::draw(Engine& eng, double left_door_anim, double right_door_anim,
                  ...
                  const std::string& anim_at_vent, const std::string& anim_in_office)
{
    ...
    eng.draw_texture(office_static_tex, 0, 0, GameSettings::OFFICE_WIDTH, GameSettings::SCREEN_HEIGHT);
    ...
    for (auto& data : projection_data)
    {
        ...
        if (src_x + src_w > 0 && src_x < office_w)
        {
            ...
            if (final_src_w > 0)
            {
                eng.draw_texture(office_tex, data.sx,
                    static_cast<int>((screen_h - target_h) / 2), 17, target_h,
                    final_src_x, 0, final_src_w, screen_h);
            }
        }
    }
}
...
{
    DrawUtils::trapezoid(eng, 22, 20, 25, 0, 0, GameSettings::OFFICE_WIDTH, 0, bw_right + hall_depth, bw_top, bw_left - hall_depth, bw_top);
    for (int i = 1; i < 10; ++i)
    {
        double prog = i / 10.0;
        int y = static_cast<int>(prog * bw_top);
        int lx = static_cast<int>((bw_left - hall_depth) * prog);
        int rx = static_cast<int>(GameSettings::OFFICE_WIDTH - (GameSettings::OFFICE_WIDTH - bw_right - hall_depth) * prog);
    ...
    for (int i = 0; i <= 5; ++i)
    {
        ...
        int bot_x = static_cast<int>((bw_left - hall_depth) + frac * (bw_right + hall_depth - bw_left + hall_depth));
...
void Office::draw_floor(Engine& eng)
{
    ...
    DrawUtils::trapezoid(eng, GameSettings::FLOOR_COLOR.r, GameSettings::FLOOR_COLOR.g, GameSettings::FLOOR_COLOR.b,
        floor_left_far, bw_bottom, floor_right_far, bw_bottom,
        GameSettings::OFFICE_WIDTH + 100, GameSettings::SCREEN_HEIGHT, -100, GameSettings::SCREEN_HEIGHT);
    ...
    for (int r = 0; r < rows; ++r)
    {
        ...
        int rx0 = static_cast<int>(floor_right_far + (GameSettings::OFFICE_WIDTH + 100 - floor_right_far) * py0);
        int lx1 = static_cast<int>(floor_left_far + (-100 - floor_left_far) * py1);
        int rx1 = static_cast<int>(floor_right_far + (GameSettings::OFFICE_WIDTH + 100 - floor_right_far) * py1);
        for (int c = 0; c < cols; ++c)
        {
            ...
            auto& color = ((r + c) % 2 == 0) ? GameSettings::FLOOR_TILE_1 : GameSettings::FLOOR_TILE_2;
            double fade = std::max(0.3, 1.0 - (1.0 - py0) * 0.6);
            DrawUtils::trapezoid(eng,
                static_cast<int>(color.r * fade), static_cast<int>(color.g * fade), static_cast<int>(color.b * fade),
                px0, y0, px1, y0, px1b, y1, px0b, y1);
        }
    }
}
...
{
    eng.draw_rect(bw_left, bw_top, bw_right - bw_left, bw_bottom - bw_top,
                  GameSettings::WALL_COLOR.r, GameSettings::WALL_COLOR.g, GameSettings::WALL_COLOR.b);
    ...
    eng.draw_rect(p1x, p1y, 128, 128, GameSettings::WALL_ACCENT.r, GameSettings::WALL_ACCENT.g, GameSettings::WALL_ACCENT.b, 255, false);
    ...
    for (int i = 0; i <= 5; ++i)
        eng.line(flyer_x + 12, flyer_y + 40 + i * 9, flyer_x + 70, flyer_y + 40 + i * 9, 74, 70, 64, 180);

    int clock_x = vp_x + 120, clock_y = bw_top + 30;
    eng.circle(clock_x, clock_y, 18, 50, 48, 55);
    eng.circle(clock_x, clock_y, 18, GameSettings::WALL_ACCENT.r, GameSettings::WALL_ACCENT.g, GameSettings::WALL_ACCENT.b, 255, false);
...
{
    int clock_x = vp_x + 120, clock_y = bw_top + 30;
    eng.line(clock_x, clock_y,
        clock_x + static_cast<int>(std::cos(t) * 10),
        clock_y + static_cast<int>(std::sin(t) * 10), 140, 130, 120);
...
{
    DrawUtils::trapezoid(eng, 38, 35, 42,
        bw_left - hall_depth, 0, bw_left, bw_top, bw_left, bw_bottom, bw_left - hall_depth, GameSettings::SCREEN_HEIGHT);
    DrawUtils::trapezoid(eng, 38, 35, 42,
        bw_right, bw_top, bw_right + hall_depth, 0, bw_right + hall_depth, GameSettings::SCREEN_HEIGHT, bw_right, bw_bottom);
}
...
void Office::draw_hallways_base(Engine& eng)
{
    ...
    eng.draw_rect(lx, ht, lw, lh, GameSettings::DOOR_FRAME_COLOR.r, GameSettings::DOOR_FRAME_COLOR.g, GameSettings::DOOR_FRAME_COLOR.b, 255, false);
    for (int i = 0; i <= 8; ++i)
    {
        int y = ht + i * (lh / 9);
    ...
    eng.draw_rect(rx, ht, rw, lh, GameSettings::DOOR_FRAME_COLOR.r, GameSettings::DOOR_FRAME_COLOR.g, GameSettings::DOOR_FRAME_COLOR.b, 255, false);
    for (int i = 0; i <= 8; ++i)
    {
        int y = ht + i * (lh / 9);
        auto c = (i % 2 == 0) ? std::array<int, 3>{168, 138, 42} : std::array<int, 3>{35, 30, 28};
...
void Office::draw_hallways_dynamic(Engine& eng, bool left_light, bool right_light,
                                   const std::string& anim_left, const std::string& anim_right)
{
    ...
    if (left_light)
    {
        eng.draw_rect(lx, ht, lw, lh, GameSettings::HALL_LIGHT.r, GameSettings::HALL_LIGHT.g, GameSettings::HALL_LIGHT.b, 70);
        eng.draw_rect(lx + 6, ht + 8, lw - 12, lh - 16, 226, 214, 170, 24);
        if (!anim_left.empty()) DrawUtils::animatronic_face(eng, anim_left, lx + 10, ht + 20, lw - 20, 160);
    }
    if (right_light)
    {
        eng.draw_rect(rx, ht, rw, lh, GameSettings::HALL_LIGHT.r, GameSettings::HALL_LIGHT.g, GameSettings::HALL_LIGHT.b, 70);
        eng.draw_rect(rx + 6, ht + 8, rw - 12, lh - 16, 226, 214, 170, 24);
        if (!anim_right.empty()) DrawUtils::animatronic_face(eng, anim_right, rx + 10, ht + 20, rw - 20, 160);
    }
}
...
void Office::draw_vent_dynamic(Engine& eng, const std::string& anim_vent)
{
    ...
    if (vent_light)
    {
        ...
        if (!anim_vent.empty()) DrawUtils::animatronic_face(eng, anim_vent, vx + 20, vy + 10, vw - 40, vh - 20);
    }
...
void Office::draw_doors(Engine& eng, double left_anim_val, double right_anim_val)
{
    ...
    if (left_anim_val > 0.01)
        draw_single_door(eng, bw_left - hall_depth - hall_w, ht, hall_w + 10, static_cast<int>(door_h * left_anim_val));
    if (right_anim_val > 0.01)
        draw_single_door(eng, bw_right + hall_depth - 10, ht, hall_w + 10, static_cast<int>(door_h * right_anim_val));
}

void Office::draw_single_door(Engine& eng, int x, int y, int w, int v_h)
{
    eng.draw_rect(x, y, w, v_h, GameSettings::DOOR_COLOR.r, GameSettings::DOOR_COLOR.g, GameSettings::DOOR_COLOR.b);
...
void Office::draw_office_elements(Engine& eng)
{
    ...
    DrawUtils::trapezoid(eng, GameSettings::DESK_TOP.r, GameSettings::DESK_TOP.g, GameSettings::DESK_TOP.b,
        bl_x, bl_y, br_x, br_y, fr_x, fr_y, fl_x, fl_y);
    auto col_desk = shade(GameSettings::DESK_COLOR, 0.85);
    DrawUtils::trapezoid(eng, col_desk.r, col_desk.g, col_desk.b,
        fl_x, fl_y, fr_x, fr_y, fr_x, fr_y + 46, fl_x, fl_y + 46);
    ...
    eng.draw_rect(mx - static_cast<int>(24 * ms), my + mh / 2 + static_cast<int>(15 * ms),
        static_cast<int>(48 * ms), static_cast<int>(5 * ms), 55, 55, 60);
...
void Office::draw_ambient(Engine& eng)
{
    ...
    for (int i = 0; i <= 6; ++i)
    {
        ...
        int a = 20 - i * 2;
        if (a > 0)
            eng.draw_rect(0, y, GameSettings::OFFICE_WIDTH, static_cast<int>(GameSettings::SCREEN_HEIGHT / 7.0), 0, 0, 0, a);
    }
}
