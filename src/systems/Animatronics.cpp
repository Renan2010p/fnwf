...
// Animatronic
Animatronic::Animatronic(std::string n, int ai, std::string start,
                         const std::unordered_map<std::string, std::vector<std::string>>& paths)
    : name(std::move(n)), ai_level(ai), position(std::move(start)), path_map(&paths), active(ai > 0) {}

void Animatronic::update(double dt, const std::string* camera_looking_at,
                           ...
                           bool left_door_closed, bool right_door_closed, bool /*mask_on*/)
{
    ...
    {
        if (!left_door_closed) { attacking = true; }
        else
        {
            auto& path = *path_map;
    ...
    {
        if (!right_door_closed) { attacking = true; }
        else
        {
            auto& path = *path_map;
    ...
    if (position == "OFFICE_VENT") return;

    auto& path = *path_map;
    auto it = path.find(position);
    if (it == path.end() || it->second.empty()) return;

    auto& possible = it->second;
    auto next_pos = possible[std::rand() % possible.size()];
    ...
    if (old_pos != next_pos)
    {
        if (name == "alice")
        {
            if (next_pos == "V" || next_pos == "OFFICE_VENT") SoundManager::play_sound("vent");
            else SoundManager::play_footstep_random();
        }
        else { SoundManager::play_footstep_random(); }
    }
    ...
    else if (next_pos == "V") { at_vent = false; }
...
auto Animatronic::is_at_left_door() const -> bool { return position == "LEFT_DOOR"; }
auto Animatronic::is_at_right_door() const -> bool { return position == "RIGHT_DOOR"; }

// Sonk
SonkAnimatronic::SonkAnimatronic(int ai)
    : ai_level(ai), active(ai > 0) {}

void SonkAnimatronic::update(double dt, const std::string* camera_looking_at,
                             bool /*left_door_closed*/, bool /*right_door_closed*/, bool /*mask_on*/)
{
    if (!active || attacking) return;
...
auto SonkAnimatronic::is_at_left_door() const -> bool { return position == "LEFT_DOOR"; }
auto SonkAnimatronic::is_at_right_door() const -> bool { return false; }
...
AnimatronicManager::AnimatronicManager(int night, const std::vector<int>* custom_ai)
    : cedro("cedro", 0, "1A", GameSettings::cedro_path()),
      ...
      alice("alice", 0, "1A", GameSettings::alice_path()),
      sonk(0)
{
    ...
    if (is_secret && custom_ai)
    {
        for (int i = 0; i < 4; ++i)
        {
            if ((*custom_ai)[i] < 40) { is_secret = false; break; }
        }
    }
    ...
    if (!custom_ai && night >= 3 && sonk_ai <= 0) sonk_ai = 5;

    sonk = SonkAnimatronic(sonk_ai);
    sonk.move_interval = secret_mode ? 1.25 : GameSettings::MOVE_INTERVAL;
    if (secret_mode) sonk.charge_duration = 0.65;
}

void AnimatronicManager::update(double dt, const std::string* camera_looking_at,
                                bool left_door_closed, bool right_door_closed, bool mask_on)
{
...
{
    return {{"cedro", cedro.position}, {"eser", eser.position},
            {"alice", alice.position}, {"sonk", sonk.position}};
}

auto AnimatronicManager::get_attacker() const -> std::string
{
    if (cedro.attacking) return "cedro";
    if (eser.attacking) return "eser";
    if (alice.attacking) return "alice";
    if (sonk.attacking) return "Sonk";
    return {};
}

auto AnimatronicManager::get_at_left_door() const -> std::string
{
    if (cedro.is_at_left_door() && !cedro.attacking) return "cedro";
    if (eser.is_at_left_door() && !eser.attacking) return "eser";
    if (alice.is_at_left_door() && !alice.attacking) return "alice";
    if (sonk.is_at_left_door() && !sonk.attacking) return "Sonk";
    return {};
}

auto AnimatronicManager::get_at_right_door() const -> std::string
{
    if (cedro.is_at_right_door() && !cedro.attacking) return "cedro";
    if (eser.is_at_right_door() && !eser.attacking) return "eser";
    if (alice.is_at_right_door() && !alice.attacking) return "alice";
    if (sonk.is_at_right_door() && !sonk.attacking) return "Sonk";
    return {};
}

auto AnimatronicManager::get_at_vent() const -> std::string
{
    if (cedro.at_vent && !cedro.attacking) return "cedro";
    if (eser.at_vent && !eser.attacking) return "eser";
    if (alice.at_vent && !alice.attacking) return "alice";
    if (sonk.at_vent && !sonk.attacking) return "Sonk";
    return {};
}

auto AnimatronicManager::get_in_office() const -> std::string
{
    if (cedro.in_office && !cedro.attacking) return "cedro";
    if (eser.in_office && !eser.attacking) return "eser";
    if (alice.in_office && !alice.attacking) return "alice";
    if (sonk.in_office && !sonk.attacking) return "Sonk";
    return {};
}

auto AnimatronicManager::check_alice_just_left() -> bool { return alice.just_left_office; }
auto AnimatronicManager::get_foxy_stage() const -> int { return sonk.stage; }
auto AnimatronicManager::is_secret_mode() const -> bool { return secret_mode; }
