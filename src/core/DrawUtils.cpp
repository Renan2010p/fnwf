...
void DrawUtils::scanlines(Engine& eng,
    ...
    std::int32_t step = 4;
    if (s_render_quality == "low") {
        a = static_cast<std::uint8_t>(a * 0.6);
        step = 6;
    } else if (alpha < 20) {
        step = 5;
    }

    std::string key = std::to_string(x) + "_" + std::to_string(y) + "_" + std::to_string(h) + "_" +
                      std::to_string(step);
...
void DrawUtils::rounded_texture(Engine& eng,
    ...
    for (std::int32_t i = 0; i <= radius; ++i) {
        auto off = static_cast<std::int32_t>(
            std::floor(radius - std::sqrt(radius * radius - (radius - i) * (radius - i))));
        if (off > 0) {
