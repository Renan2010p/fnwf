...
bool EnginePS2::init(std::string_view /*title*/, std::uint32_t w, std::uint32_t h,
    ...
    // Request hardware surface with double buffering for GPU-accelerated rendering
    ...
    // Debug: check if hardware flags were applied
    std::fprintf(stderr, "DEBUG: SDL flags=0x%08X HWSURFACE=%d DOUBLEBUF=%d HWACCEL=%d\n", 
                 m_screen->flags,
                 (m_screen->flags & SDL_HWSURFACE) ? 1 : 0,
                 (m_screen->flags & SDL_DOUBLEBUF) ? 1 : 0,
                 (m_screen->flags & SDL_HWACCEL) ? 1 : 0);
